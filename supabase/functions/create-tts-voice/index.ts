  ...
  try {
    ...
    if (parsed.detail && typeof parsed.detail === "object") {
      ...
      if (
        ...
        return "The configured ElevenLabs API key cannot create voice clones on its current plan. Upgrade the ElevenLabs workspace to a paid plan with instant voice cloning, or replace ELEVEN_LABS_API with a key that already has that feature.";
      }
...
serve(async (req) => {
  ...
  try {
    ...
    if (userError || !userData?.user) {
      console.error("create-tts-voice auth error:", userError?.message ?? userError);
    ...
    if (files.length === 0) {
      return new Response(JSON.stringify({ error: "Add at least one audio sample to create a voice clone." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!elevenLabsResponse.ok) {
      return new Response(JSON.stringify({
        error: getElevenLabsErrorMessage(responseText, "Failed to create voice clone"),
      }), {
